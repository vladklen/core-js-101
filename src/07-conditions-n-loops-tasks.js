...
function doRectanglesOverlap(rect1, rect2) {
  const coord1 = [rect1.left, rect1.top, rect1.left + rect1.width, rect1.top + rect1.height];
  const coord2 = [rect2.left, rect2.top, rect2.left + rect2.width, rect2.top + rect2.height];
  if (coord2[0] >= coord1[2] || coord2[2] <= coord1[1] || coord2[1] >= coord1[3]
    || coord2[3] <= coord1[1]) {
    return false;
  }
  return true;
...
function findFirstSingleChar(str) {
  return str.split('').filter((el, index, array) => array.indexOf(el) === array.lastIndexOf(el))[0];
}
...
function reverseString(str) {
  return str.split(' ').map((i) => (i.split('').reverse().join(''))).reverse().join(' ');
}
...
function getDigitalRoot(num) {
  return Array.from(String(num)).reduce((acc, el) => acc + Number(el), 0).toString().split('')
...
function getCommonDirectoryPath(pathes) {
  const separator = '/';
  const parts = pathes.map((path) => path.split(separator));
  ...
  let commonPath = commonParts.join(separator);


  if (pathes.every((elem) => elem[0] === '/')) {
    commonPath = `${commonPath}${separator}`;
  } else {
    commonPath = '';
  }
  return commonPath;
